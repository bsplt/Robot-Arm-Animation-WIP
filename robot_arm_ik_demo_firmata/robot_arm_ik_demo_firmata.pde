...
class Motor
{
  ...
  Motor(float stepAngle_, boolean reverseDirection, int stepPin_, int dirPin_)
  {
  ...
  void step(boolean forwardDirection)
  {
    ...
    if (rotationalPos + stepAngle * direction > lowerConstraint && rotationalPos + stepAngle * direction < upperConstraint)
    { 
      rotationalPos += stepAngle * direction;
      if (stepDirection == -1)
        forwardDirection = !forwardDirection;
      arduino.digitalWrite(dirPin, forwardDirection ? 1 : 0);
